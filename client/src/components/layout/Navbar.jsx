    ...
    <nav className="bg-indigo-600 shadow-md">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          <div className="shrink-0 flex items-center">
            <Link to="/" onClick={closeMenu} className="text-white font-bold text-xl tracking-wider">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-indigo-700 px-2 pt-2 pb-3 space-y-1 sm:px-3 shadow-inner">
           {!isAuthenticated ? (
             <>
                <Link to="/" onClick={closeMenu} className="text-indigo-100 hover:text-white hover:bg-indigo-600 block px-3 py-2 rounded-md text-base font-medium">Home</Link>
                <Link to="/login" onClick={closeMenu} className="text-white bg-indigo-500 hover:bg-indigo-400 block px-3 py-2 rounded-md text-base font-bold mt-4">Login / Sign Up</Link>
             </>
           ) : (
             <>
                {user?.role === 'admin' && (
                  <Link to="/admin" onClick={closeMenu} className="text-amber-900 bg-amber-400 hover:bg-amber-300 block px-3 py-2 rounded-md text-base font-black mb-4">
                    Admin Control Center
                  </Link>
                )}
                <Link to="/dashboard" onClick={closeMenu} className="text-indigo-100 hover:text-white hover:bg-indigo-600 block px-3 py-2 rounded-md text-base font-medium">Dashboard</Link>
                <Link to="/auctions" onClick={closeMenu} className="text-indigo-100 hover:text-white hover:bg-indigo-600 block px-3 py-2 rounded-md text-base font-medium">Auctions</Link>
                <Link to="/auctions/create" onClick={closeMenu} className="text-indigo-100 hover:text-white hover:bg-indigo-600 block px-3 py-2 rounded-md text-base font-medium">Create Auction</Link>
                <Link to="/my-auctions" onClick={closeMenu} className="text-indigo-100 hover:text-white hover:bg-indigo-600 block px-3 py-2 rounded-md text-base font-medium">My Auctions</Link>
                <Link to="/my-bids" onClick={closeMenu} className="text-indigo-100 hover:text-white hover:bg-indigo-600 block px-3 py-2 rounded-md text-base font-medium">My Bids</Link>
                <Link to="/profile" onClick={closeMenu} className="text-indigo-100 hover:text-white hover:bg-indigo-600 block px-3 py-2 rounded-md text-base font-medium">My Profile</Link>
